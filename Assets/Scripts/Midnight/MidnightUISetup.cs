...
/// 
/// Updated for @midnight-ntwrk/dapp-connector-api v4.0.1
/// - Clear Midnight vs Cardano mode indication
/// - Supports shielded/unshielded balance display
/// - Network-aware connection (mainnet, preprod, preview)
/// - Modern UI with prominent connection mode display
/// 
...
public class MidnightUISetup : MonoBehaviour
{
    ...
    public Color midnightAccentColor = new Color(0.4f, 0.2f, 0.8f, 1f); // Purple for Midnight
    public Color cardanoAccentColor = new Color(0.2f, 0.5f, 0.9f, 1f); // Blue for Cardano
    public Color successColor = new Color(0.2f, 0.8f, 0.4f, 1f);
    public Color warningColor = new Color(1f, 0.7f, 0.2f, 1f);
    public Color textColor = Color.white;
    public Color dimTextColor = new Color(0.6f, 0.6f, 0.7f, 1f);
    ...
    public void CreateMidnightUI()
    {
        ...
        // ============================================================
        // HEADER SECTION - Connection Mode Badge
        // ============================================================
        
        // Connection mode badge (prominent indicator: MIDNIGHT or CARDANO)
        GameObject modeBadgeObj = CreatePanel(panelObj.transform, "ModeBadge", new Color(0.2f, 0.2f, 0.3f, 1f));
        RectTransform modeBadgeRect = modeBadgeObj.GetComponent<RectTransform>();
        modeBadgeRect.anchorMin = new Vector2(0.5f, 1);
        modeBadgeRect.anchorMax = new Vector2(0.5f, 1);
        modeBadgeRect.pivot = new Vector2(0.5f, 1);
        modeBadgeRect.sizeDelta = new Vector2(280, 40);
        modeBadgeRect.anchoredPosition = new Vector2(0, -15);
        Image modeBadgeImage = modeBadgeObj.GetComponent<Image>();
        
        GameObject modeBadgeTextObj = CreateText(modeBadgeObj.transform, "ModeBadgeText", "NOT CONNECTED", 18, TextAnchor.MiddleCenter);
        RectTransform modeBadgeTextRect = modeBadgeTextObj.GetComponent<RectTransform>();
        modeBadgeTextRect.anchorMin = Vector2.zero;
        modeBadgeTextRect.anchorMax = Vector2.one;
        modeBadgeTextRect.offsetMin = Vector2.zero;
        modeBadgeTextRect.offsetMax = Vector2.zero;
        Text modeBadgeText = modeBadgeTextObj.GetComponent<Text>();
        modeBadgeText.fontStyle = FontStyle.Bold;

        // Create title
        GameObject titleObj = CreateText(panelObj.transform, "Title", "Wallet Connection", 24, TextAnchor.MiddleCenter);
        ...
        // Create status text
        GameObject statusObj = CreateText(panelObj.transform, "StatusText", "Detecting wallet...", 14, TextAnchor.MiddleCenter);
        RectTransform statusRect = statusObj.GetComponent<RectTransform>();
        statusRect.anchorMin = new Vector2(0, 1);
        statusRect.anchorMax = new Vector2(1, 1);
        statusRect.pivot = new Vector2(0.5f, 1);
        statusRect.sizeDelta = new Vector2(0, 25);
        statusRect.anchoredPosition = new Vector2(0, -95);
        Text statusText = statusObj.GetComponent<Text>();
        statusText.color = dimTextColor;

        // Create address row container (positioned after balance section)
        ...
        addressRowRect.sizeDelta = new Vector2(-40, 30);
        addressRowRect.anchoredPosition = new Vector2(0, -258);

        // Create address text (clickable area)
        GameObject addressObj = CreateText(addressRowObj.transform, "AddressText", "", 12, TextAnchor.MiddleLeft);
        RectTransform addressRect = addressObj.GetComponent<RectTransform>();
        addressRect.anchorMin = new Vector2(0, 0);
        addressRect.anchorMax = new Vector2(0.82f, 1);
        addressRect.offsetMin = Vector2.zero;
        addressRect.offsetMax = Vector2.zero;
        Text addressText = addressObj.GetComponent<Text>();
        addressText.color = dimTextColor;
        ...
        GameObject connectBtnObj = CreateButton(panelObj.transform, "ConnectButton", "Connect Wallet", midnightAccentColor);
        ...
        connectBtnRect.sizeDelta = new Vector2(200, 50);
        connectBtnRect.anchoredPosition = new Vector2(0, -125);
        Button connectButton = connectBtnObj.GetComponent<Button>();
        Text connectButtonText = connectBtnObj.GetComponentInChildren<Text>();
        connectButtonText.fontSize = 18;
        ...
        // Network badge (shows which network: Midnight Preprod, Cardano Preprod, etc.)
        GameObject networkBadgeObj = CreatePanel(panelObj.transform, "NetworkBadge", new Color(0.15f, 0.15f, 0.22f, 1f));
        RectTransform networkBadgeRect = networkBadgeObj.GetComponent<RectTransform>();
        networkBadgeRect.anchorMin = new Vector2(0.5f, 1);
        networkBadgeRect.anchorMax = new Vector2(0.5f, 1);
        networkBadgeRect.pivot = new Vector2(0.5f, 1);
        networkBadgeRect.sizeDelta = new Vector2(220, 28);
        networkBadgeRect.anchoredPosition = new Vector2(0, -185);
        
        GameObject networkObj = CreateText(networkBadgeObj.transform, "NetworkText", "Network: --", 13, TextAnchor.MiddleCenter);
        RectTransform networkRect = networkObj.GetComponent<RectTransform>();
        networkRect.anchorMin = Vector2.zero;
        networkRect.anchorMax = Vector2.one;
        networkRect.offsetMin = Vector2.zero;
        networkRect.offsetMax = Vector2.zero;
        Text networkText = networkObj.GetComponent<Text>();
        networkText.color = dimTextColor;

        // Balance text (main native balance)
        GameObject balanceObj = CreateText(panelObj.transform, "BalanceText", "Balance: --", 22, TextAnchor.MiddleCenter);
        ...
        balanceRect.anchoredPosition = new Vector2(0, -220);
        Text balanceText = balanceObj.GetComponent<Text>();
        balanceText.color = successColor;
        ...
        float sendY = -295f; // Start position for send section
        ...
        dividerRect.anchoredPosition = new Vector2(0, sendY);

        // Send section title
        GameObject sendTitleObj = CreateText(panelObj.transform, "SendTitle", "Send Transaction", 18, TextAnchor.MiddleCenter);
        ...
        sendTitleRect.sizeDelta = new Vector2(0, 30);
        sendTitleRect.anchoredPosition = new Vector2(0, sendY - 10);

        // Recipient label
        GameObject recipientLabelObj = CreateText(panelObj.transform, "RecipientLabel", "Recipient Address:", 12, TextAnchor.MiddleLeft);
        ...
        recipientLabelRect.sizeDelta = new Vector2(-40, 20);
        recipientLabelRect.anchoredPosition = new Vector2(0, sendY - 42);
        ...
        recipientInputRect.sizeDelta = new Vector2(-40, 32);
        recipientInputRect.anchoredPosition = new Vector2(0, sendY - 65);
        InputField recipientInput = recipientInputObj.GetComponent<InputField>();

        // Amount label
        GameObject amountLabelObj = CreateText(panelObj.transform, "AmountLabel", "Amount (lovelace):", 12, TextAnchor.MiddleLeft);
        ...
        amountLabelRect.sizeDelta = new Vector2(-40, 20);
        amountLabelRect.anchoredPosition = new Vector2(0, sendY - 100);
        ...
        amountInputRect.sizeDelta = new Vector2(-40, 32);
        amountInputRect.anchoredPosition = new Vector2(0, sendY - 122);
        ...
        GameObject txStatusObj = CreateText(panelObj.transform, "TxStatusText", "", 12, TextAnchor.MiddleCenter);
        ...
        txStatusRect.sizeDelta = new Vector2(-40, 22);
        txStatusRect.anchoredPosition = new Vector2(0, sendY - 158);
        Text txStatusText = txStatusObj.GetComponent<Text>();
        txStatusText.color = warningColor;

        // Send button
        GameObject sendBtnObj = CreateButton(panelObj.transform, "SendButton", "Send", successColor);
        ...
        sendBtnRect.sizeDelta = new Vector2(140, 40);
        sendBtnRect.anchoredPosition = new Vector2(0, sendY - 185);
        Button sendButton = sendBtnObj.GetComponent<Button>();

        // === TAB BUTTONS ===
        float tabY = sendY - 235f; // -530
        ...
        bridge.modeBadgeText = modeBadgeText;
        bridge.modeBadgeImage = modeBadgeImage;
        bridge.midnightColor = midnightAccentColor;
        bridge.cardanoColor = cardanoAccentColor;
