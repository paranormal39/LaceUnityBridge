...
/// 
/// Updated for @midnight-ntwrk/dapp-connector-api v4.0.0
/// 
/// This script handles:
/// - Detecting if Lace wallet is available in the browser
/// - Connecting to Lace wallet via connect(networkId)
/// - Displaying connection status and shield address
/// - Handling v4.0.0 granular wallet state (shieldedBalances, unshieldedBalances, dustBalance)
/// 
...
public class MidnightBridge : MonoBehaviour
{
    ...
    // Midnight connection debug (step-by-step connector inspection)
    [DllImport("__Internal")]
    private static extern void DebugMidnightConnection();
    ...
    [Header("Connection Mode Display")]
    [Tooltip("Text showing connection mode (MIDNIGHT PREPROD, CARDANO, etc.)")]
    public Text modeBadgeText;

    [Tooltip("Image background for mode badge (changes color based on mode)")]
    public Image modeBadgeImage;

    [Tooltip("Color for Midnight mode")]
    public Color midnightColor = new Color(0.4f, 0.2f, 0.8f, 1f);

    [Tooltip("Color for Cardano mode")]
    public Color cardanoColor = new Color(0.2f, 0.5f, 0.9f, 1f);
    ...
    /// <summary>
    /// Run the full Midnight connection debug (step-by-step).
    /// Attempts to find connector, call enable(), state(), serviceUriConfig().
    /// All output goes to browser console with [MidnightDebug] prefix.
    /// Call this from a UI button or inspector for troubleshooting.
    /// </summary>
    public void RunMidnightConnectionDebug()
    {
#if UNITY_WEBGL && !UNITY_EDITOR
        Debug.Log("[MidnightBridge] Running Midnight connection debug...");
        DebugMidnightConnection();
#else
        Debug.Log("[MidnightBridge] Midnight connection debug only works in WebGL builds");
#endif
    }
    ...
    public void Disconnect()
    {
        ...
        if (networkText != null) networkText.text = "Network: --";
    ...
    public void OnConnectionSuccess(string resultJson)
    {
        ...
        // Update mode badge prominently
        UpdateModeBadge(mode, network);
    ...
    public void OnConnectionError(string errorMessage)
    {
        ...
        string displayError = errorMessage.Length > 50 
    ...
    private void UpdateConnectButtonText()
    {
        ...
        {
            connectButtonText.text = isConnected ? "Disconnect" : "Connect Wallet";
        }
    }

    /// <summary>
    /// Update the prominent mode badge to show MIDNIGHT or CARDANO connection.
    /// </summary>
    private void UpdateModeBadge(string mode, string network)
    {
        if (modeBadgeText == null) return;
        
        bool isMidnight = mode == "midnight";
        string networkUpper = string.IsNullOrEmpty(network) ? "" : network.ToUpper();
        
        if (isMidnight)
        {
            modeBadgeText.text = $"⬢ MIDNIGHT {networkUpper}";
            if (modeBadgeImage != null)
            {
                modeBadgeImage.color = midnightColor;
            }
        }
        else
        {
            modeBadgeText.text = $"◈ CARDANO {networkUpper}";
            if (modeBadgeImage != null)
            {
                modeBadgeImage.color = cardanoColor;
            }
        }
    }

    /// <summary>
    /// Reset the mode badge to disconnected state.
    /// </summary>
    private void ResetModeBadge()
    {
        if (modeBadgeText != null)
        {
            modeBadgeText.text = "NOT CONNECTED";
        }
        if (modeBadgeImage != null)
        {
            modeBadgeImage.color = new Color(0.2f, 0.2f, 0.3f, 1f);
        }
    }
    ...
    /// }
    /// 
    /// Legacy format: { "address": "...", "balances": { "native": "123" } }
    /// </summary>
    public void OnWalletStateSuccess(string stateJson)
    {
        ...
        try
        {
            ...
            {
                // v4.0.0: Check for shieldedBalances first
                int shieldedBalancesStart = stateJson.IndexOf("\"shieldedBalances\":");
                if (shieldedBalancesStart >= 0)
                {
                    Debug.Log("[MidnightBridge] Parsing v4.0.0 wallet state format");
                    
                    // Parse shielded balances
                    int balStart = stateJson.IndexOf("{", shieldedBalancesStart);
                    if (balStart >= 0)
                    {
                        int balEnd = FindMatchingBrace(stateJson, balStart + 1);
                        if (balEnd > balStart)
                        {
                            string balancesStr = stateJson.Substring(balStart + 1, balEnd - balStart - 1);
                            ParseBalances(balancesStr);
                        }
                    }
                    
                    // Also check for dustBalance
                    int dustStart = stateJson.IndexOf("\"dustBalance\":\"");
                    if (dustStart >= 0)
                    {
                        dustStart += 15;
                        int dustEnd = stateJson.IndexOf("\"", dustStart);
                        if (dustEnd > dustStart)
                        {
                            string dustBalance = stateJson.Substring(dustStart, dustEnd - dustStart);
                            tokenBalances["dust"] = dustBalance;
                            Debug.Log($"[MidnightBridge] Dust balance: {dustBalance}");
                        }
                    }
                    
                    // Update shieldAddress if present in new format
                    int addrStart = stateJson.IndexOf("\"shieldedAddress\":\"");
                    if (addrStart >= 0)
                    {
                        addrStart += 19;
                        int addrEnd = stateJson.IndexOf("\"", addrStart);
                        if (addrEnd > addrStart)
                        {
                            shieldAddress = stateJson.Substring(addrStart, addrEnd - addrStart);
                            if (addressText != null)
                            {
                                addressText.text = TruncateAddress(shieldAddress);
                            }
                        }
                    }
                }
                ...
                {
                    // Legacy format: Extract balances object
                    int balancesStart = stateJson.IndexOf("\"balances\":{");
                    if (balancesStart >= 0)
                    {
                        Debug.Log("[MidnightBridge] Parsing legacy wallet state format");
